...
func generatePreMergeChain(n int) (*core.Genesis, []*types.Block) {
	...
	genesis := &core.Genesis{
		Config:     config,
		Alloc:      core.GenesisAlloc{testAddr: {Balance: testBalance}, testValidatorAddr: {Balance: testBalance}},
...
func TestValidateBuilderSubmissionV2_CoinbasePaymentDefault(t *testing.T) {
	...
	// this transaction sends 3 wei from the proposer fee recipient to the proposer fee recipient, this should not count as a profit
	// but its tip of baseFee should count as a profit
	...
	// this tx sends 11 wei from the proposer fee recipient to the proposer fee recipient, this should not affect profit
...
func TestValidateBuilderSubmissionV2_Blocklist(t *testing.T) {
	...
	nonce = statedb.GetNonce(testValidatorAddr)
	tx, _ = types.SignTx(types.NewTransaction(nonce, testAddr, big.NewInt(10), 21000, baseFee, nil), signer, testValidatorKey)
	blockedTxs = append(blockedTxs, tx)
